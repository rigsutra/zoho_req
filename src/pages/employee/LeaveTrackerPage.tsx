      ...
      <Tabs defaultValue="my-data" className="w-full">
        <div className="flex items-center justify-between mb-4">
          <TabsList className="h-12 bg-transparent p-0 gap-6">
            <TabsTrigger
              value="my-data"
              className="px-0 data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none h-full"
            >
              My Data
            </TabsTrigger>
            <TabsTrigger
...
function MyDataSection({ yearStats, dateRange, setDateRange, balances }: any) {
  return (
    <Tabs defaultValue="summary" className="w-full">
      <TabsList className="bg-gray-100/50 p-1 rounded-lg w-auto inline-flex h-auto">
        <TabsTrigger
          value="summary"
      ...
      {balances === undefined ? (
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {balances.map((bal: any) => {
            const IconComponent =
              leaveIcons[bal.leaveType?.code] || CalendarIcon;
      ...
      <CardContent>
        {balances === undefined ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {balances.map((bal: any) => (
              <div key={bal._id} className="rounded-xl border p-6 bg-gray-50/30">
                <div className="flex items-center justify-between mb-3">
                  <span className="font-semibold">{bal.leaveType?.name}</span>
                  <Badge variant="outline">{bal.leaveType?.code}</Badge>
                </div>
                <div className="text-3xl font-light mb-1">{bal.available}</div>
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  <div className="flex justify-between">
                    <span>Allocated</span>
                    <span>{bal.totalAllocation}</span>
                  </div>
                  {bal.carriedForward > 0 && (
                    <div className="flex justify-between">
                      <span>Carried Forward</span>
                      <span>{bal.carriedForward}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span>Used</span>
                ...
                <div className="mt-3 h-2 rounded-full bg-muted overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        leaveColors[bal.leaveType?.code ?? ""] ?? "#6366f1",
                    }}
                  />
                </div>
              </div>
...
function TeamSection() {
  ...
  const eventStyleGetter = (event: (typeof events)[0]) => ({
    style: {
      backgroundColor:
        leaveColors[event.resource.leaveTypeCode ?? ""] ?? "#6366f1",
      borderRadius: "4px",
      opacity: 0.9,
      color: "white",
      border: "0",
  ...
  return (
    <div className="space-y-6">
      <Card className="border-none shadow-sm">
        <CardHeader>
          <CardTitle>Team Members On Leave This Week</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {leave.leaveType?.name} &middot; {leave.startDate} to{" "}
                      {leave.endDate}
                    </p>
                  </div>
...
function HolidaysSection() {
  const holidays = useQuery(api.holidays.getForYear, {
        ...
        {holidays === undefined ? (
          ...
          <div className="space-y-3">
            {holidays.map((holiday: any) => (
              <div
                key={holiday._id}
                className="flex items-center justify-between p-4 rounded-xl border bg-gray-50/30 hover:bg-gray-50 transition-colors"
              >
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 rounded-lg bg-primary/10 text-primary flex items-center justify-center text-lg font-semibold">
                    {new Date(holiday.date).toLocaleDateString("en-US", {
                      day: "2-digit",
                    })}
                  </div>
